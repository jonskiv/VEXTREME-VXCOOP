...
VEXTREME-VXCOOP is a fork of [technobly/VEXTREME](https://github.com/technobly/VEXTREME), the base multicart hardware and firmware. This fork adds VXT, a cooperative dual-CPU game framework built on top of that hardware, plus VX-COOP and a handful of smaller example carts that demonstrate it. That includes `code/stm32/vxt/` and `code/app6809/vxt/` (the toolkit itself), `code/stm32/gamelib/` (STM32-side drawing math built on it), the VOOM port (`code/stm32/game/voom_smart.c` and `code/app6809/VOOM/`), and every example cart (`code/stm32/vxcoop/` and `code/app6809/{VXCOOP,test1_led,test6c,test8_cal}/`) — see the [VXT Toolkit](#vxt-toolkit) section further down for the full list. The multicart hardware, the menu, and the base firmware everywhere else in this repo are the upstream project this fork is built on.

These demo files are intended as a proof of concept, so while they may prove useful, they are provided AS-IS without any warranty.  They are likely corrections and enhancements to be made, but offered only for visibility and potential learning. Please do your own research. Besides the examples here, I was able to demonstrate a working game demo using similar routines here in a working development game called [Falcon Wars: Corellian Dog Fights](https://caeolotronics.blogspot.com/2026/09/new-game-in-development-falcon-wars.html).

**Start here: [docs/VX-COOP/](docs/VX-COOP/)** — the reference guide (architecture, protocol, every hard-won rule) and the function-level reference for everything the toolkit provides. The [VXT Toolkit](#vxt-toolkit) section below is a short map of what's in this fork; the docs folder is where it's actually explained. Note the *nota bene* near the top of that guide: its descriptions of prior work (VEXTREME's existing firmware, Sprite_tm's original VOOM) come from reading that code, not from its authors, and should be independently validated.
