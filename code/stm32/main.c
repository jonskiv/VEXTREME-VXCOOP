...
// VXT toolkit
#include "vxt/vxt_rpc.h"
#include "vxt/vxt_frame.h"
#include "vxt/vxt_input.h"
#include "vxt/vxt_sound.h"
#include "vxt/vxt_smart.h"
#include "vxt/vxt_smart_text.h"
#include "vxt/vxt_font_western.h"
#include "vxt/vxt_bounds.h"
#include "vxt/test6c_handler.h"      /* TEST 6c (RPC 71) */
#if VXT_ENABLE_VXCOOP
#include "vxcoop/vxcoop_handler.h"   /* VX-COOP toolkit demonstration cart -
                                   * docs/VX-COOP/, code/app6809/VXCOOP/ */
#endif
#if VXT_ENABLE_CAL
#include "vxt/vxt_cal.h"
#endif
#if VXT_ENABLE_VOOM
#include "vxt/voom_smart_handler.h"  /* VOOM ported to vxt_smart - a from-
                                      * scratch parallel game, NOT the
                                      * original vxt_draw-based port */
#endif
...
/* VXT toolkit: RPC 69 is a ONE-SHOT the 6809 fires at startup, publishing
 * every SmartList routine address (up to 13 addresses, 26 bytes) via
 * parmRam; vxtSmartSetAddrs() stores them. Emitting before this handshake
 * is a documented no-op (vxtSmartReady()==0), which fails as a blank
 * screen, not a 6809 jump into garbage. Shared by every app that uses
 * vxt_smart - not every app publishes every address; an app that never
 * calls the corresponding vxtSmart*() function simply never dispatches
 * through its leftover parmRam garbage, so an unpublished slot is
 * harmless. */
static void vxt_smart_addrs_handler(uint8_t id, volatile uint8_t *parm) {
    vxtSmartAddrs a;
    (void)id;
    a.setScale     = (uint16_t)((parm[0]  << 8) | parm[1]);
    a.setIntensity = (uint16_t)((parm[2]  << 8) | parm[3]);
    a.recenter     = (uint16_t)((parm[4]  << 8) | parm[5]);
    a.startMove    = (uint16_t)((parm[6]  << 8) | parm[7]);
    a.startDraw    = (uint16_t)((parm[8]  << 8) | parm[9]);
    a.cont         = (uint16_t)((parm[10] << 8) | parm[11]);
    a.end          = (uint16_t)((parm[12] << 8) | parm[13]);
    a.startMoveBig = (uint16_t)((parm[14] << 8) | parm[15]);
    a.startDrawBig = (uint16_t)((parm[16] << 8) | parm[17]);
    a.startDrawHuge = (uint16_t)((parm[18] << 8) | parm[19]);
    a.setScaleHi    = (uint16_t)((parm[20] << 8) | parm[21]);
    a.startDrawHuge16 = (uint16_t)((parm[22] << 8) | parm[23]);
    a.startDraw32   = (uint16_t)((parm[24] << 8) | parm[25]);
    vxtSmartSetAddrs(&a);
}
...
void doHandleEvent(int data) {
    ...
    switch (data) {
        ...
        /* VXT toolkit: route only the reserved 64-79 block to the
         * toolkit dispatcher. Every other unmatched ID stays a silent
         * no-op, byte-for-byte matching original firmware behavior. */
        default:
...
// #endif
    ...
    // VXT toolkit - register the reserved RPC IDs 64-79 in use here.
    vxtRpcInit();
    vxtSmartTextSetFontTable(vxtFontWestern, vxtFontWesternCount);
    vxtRpcRegister(71, test6c_handler);                       /* 71: Test 6c (multi-channel sound) */
    vxtRpcRegister(69, vxt_smart_addrs_handler);              /* 69: SmartList addr publish - shared by
                                                                 * every app that uses vxt_smart */
#if VXT_ENABLE_CAL
    vxtRpcRegister(75, vxt_cal_handler);                      /* 75: calibration/measurement rig (code/app6809/test8_cal) */
    vxtRpcRegister(76, vxt_cal_init_handler);                 /* 76: cal rig one-shot init - a 6809 warm reset does NOT
                                                                 * reset the STM32, so this resets the rig's own state */
#endif
#if VXT_ENABLE_VOOM
    vxtRpcRegister(77, voom_smart_handler);                   /* 77: VOOM ported to vxt_smart (code/app6809/VOOM) */
#endif
#if VXT_ENABLE_VXCOOP
    vxtRpcRegister(78, vxcoop_handler);                        /* 78: VX-COOP toolkit demonstration (code/app6809/VXCOOP) */
    vxtRpcRegister(79, vxcoop_init_handler);                   /* 79: VX-COOP one-shot init - same 6809-warm-reset reason as 76 */
#endif
