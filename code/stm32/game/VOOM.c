...
*/
/*
 * VOOM (vxt_smart port). Sprite_tm's original platform-independent Doom BSP/
 * projection renderer. It only calls the external veLineAdd(x1,y1,x2,y2) and
 * never touches the wire format, so it has nothing to do with the vxt_draw ->
 * vxt_smart port - see code/stm32/game/voom_smart.c for the part that changed.
 * Kept as a separate copy (not a shared #include of code/stm32/voom.c) so the
 * original Test 7 VOOM stays untouched for comparison, per project decision.
 *
 * REGENERATED mechanically from code/stm32/voom.c, reverting the
 * SD-streaming rewrite this file used to carry (every dmline[i]/dmseg[i]/
 * dmnode[i] had become a voomGetLine()/voomGetSegRun()/voomGetNode() cache
 * fetch). The lumps live in flash now (game/voom_level_data.S), so the direct
 * indexing below is correct again and costs nothing. Exactly three deliberate
 * differences from voom.c remain:
 *   1. DoomVertex/Line/Seg/Bbox/Node moved to VOOM.h (voom_smart.c names them
 *      too); DoomSide/DoomSsec/DoomSec stay private here;
 *   2. zbm/zbmx are caller-bound pointers, not bss arrays - voomBindArena();
 *   3. the desktop-debug trace calls in voomHandleKeys() are removed - they
 *      print to nothing here, and linking newlib's formatter (with %f) costs
 *      several KB on a build with ~8KB of flash headroom.
 */
/*
...
/* Zmap bitbuffer + full-column shortcut row. Changed from plain
 * arrays to caller-bound pointers - see voomBindArena() in VOOM.h for why
 * (8,448 bytes of bss this build does not have). Every index below is
 * unchanged. */
static unsigned int *zbm;
static char *zbmx;
